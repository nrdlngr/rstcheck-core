...
def check_file(
    ...
    with _sphinx.load_sphinx_if_available(source_file) as custom_substitutions:
        # Handle substitutions from conf.py if they were returned
        if custom_substitutions:
            logger.debug("Adding %s substitutions from conf.py", len(custom_substitutions))
            ignore_dict["substitutions"].extend(custom_substitutions)
